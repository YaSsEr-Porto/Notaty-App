...
class FavouritesFragment : Fragment() {
    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View {
        ...
        binding.favRv.adapter = noteAdapter

        noteViewModel.noteList.observe(viewLifecycleOwner) { notes ->
            fullNoteList = notes.filter { it.isBookmarked }
            noteAdapter.submitList(fullNoteList)
            binding.favTemp.visibility = if (fullNoteList.isEmpty()) View.VISIBLE else View.GONE
        }
